...
function main() {
  ...
  var numindex,
    num,
    esc = false;
  (esc_bracket = false), (esc_paren_open = false), (esc_question = false), (lineDrawing = false), (cursorKeys = false);
  var textattrs = {
    bold: false,
    inverse: false,
  ...
  function process_attrs(code) {
    if(code == 1) {
      ...
      } else if((code >= 30 && code < 38) || (code >= 90 && code < 98)) {
  ...
  function recolorTerminal() {
    ...
    for(var i = 0; i < maxy; i++) {
      for(var j = 0; j < maxx; j++) {
        ...
        chars[i][j].style.backgroundColor = localStorage.bgColor;
      }
    }
    elTerm.style.backgroundColor = localStorage.bgColor;
  }
  function setChar(x, y, ch) {
    ...
    if(textattrs.inverse) {
      ...
      chars[y][x].style.color = textattrs.color;
      chars[y][x].style.backgroundColor = textattrs.backgroundColor;
    }
    chars[y][x].textContent = lineDrawing ? (specialCharacters.hasOwnProperty(ch) ? specialCharacters[ch] : ch) : ch;
  }
  function ctrl(ch) {
          ...
          case '@':
            for(var i = 0; i < val; i++) {
              for(var j = maxx - 2; j >= curx; j--) {
                ...
                chars[cury][j + 1].style.color = chars[cury][j].style.color;
          ...
          case 'P':
            for(var i = 0; i < val; i++) {
              for(var j = curx; j < maxx - 1; j++) {
                ...
                chars[cury][j].style.color = chars[cury][j + 1].style.color;
  ...
  sock.ondata = function(e) {
    ...
    ignoreChars = oldDataLength < ignoreChars ? oldDataLength - ignoreChars : 0;

...
function showOptions(inputCallback, recolorTerminal) {
  ...
  for(var i = 0; i < 12; i++) {
    ...
    if(i % 3 == 2) specialKeys.appendChild(document.createElement('br'));
  }
  specialCharacterContainers[0].focus();
  function updateSpecialKeys() {
    for(var i = 0; i < 12; i++) {
      specialCharacterContainers[i].textContent = keyPages[pageIndex][i];
    }
  }
  updateSpecialKeys();
    ...
    if(!isNaN(e.key) && e.key != '0') {
      ...
      switch (e.key) {
        case 'ArrowUp':
          ...
          break;
        case 'ArrowDown':
          if(activeChar < 9 && activeChar >= -6) activeChar += 3;
          break;
        case 'ArrowLeft':
          if(activeChar > 0) activeChar--;
          break;
        case 'ArrowRight':
          if(activeChar < 11 && activeChar >= 0) activeChar++;
          break;
      }
      e.preventDefault();
      if(activeChar >= 0) specialCharacterContainers[activeChar].focus();
      else {
        if(activeChar < -3) bgPicker.focus();
        else fgPicker.focus();
      }
